...
\begin{definition}
A \alert{serial} system consists of $K$ components $c_{1}, \ldots, c_{K}$ arranged in such a way that the system works if an only if {\bf all} of the components function properly.
\end{definition}
...
\begin{enumerate}
\item Serial components A B can be replace with a component F that operates with probability $P(A\cap B)=(0.92)^2=0.8464$.
