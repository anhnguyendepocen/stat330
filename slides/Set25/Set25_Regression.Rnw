...
sum of squared ``errors'' (SSE), $\sum_{i=1}^n r_i^2$, 
...
\[ \begin{array}{rl}
\mbox{assumption} & \mbox{true model} \pause \\
\hline
...
sim <- data.frame(x = runif(100, 0, 3)) %>%
  ...
  ungroup() %>%
  mutate(violation = paste("Violation:", violation))
@
...
ggplot(sim, aes(.fitted, .resid)) + 
  geom_point() +
  facet_wrap(~violation, scales="free") +
...
\[ 
R^2 = r^2 = 1-\frac{SSE}{SST} = 1-\frac{SSE}{SYY} = 1-\frac{\sum_{i=1}^n e_i^2}{\sum_{i=1}^n (y_i-\overline{y})^2}.
\]
