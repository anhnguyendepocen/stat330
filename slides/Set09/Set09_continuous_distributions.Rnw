...
\[ 
f(x) = \left\{ \begin{array}{ll} \frac{\lambda^\alpha}{\Gamma(\alpha)}x^{\alpha-1}e^{-\lambda x} & x>0 \\
0 & mbox{otherwise} \end{array} \right.
\]
with shape parameter $\alpha>0$ and rate parameter $\lambda>0$ 
\pause
where 
\[ \Gamma(\alpha) = \int_0^\infty x^{\alpha-1}e^{-\alpha} dx \]
\pause 
...
F(x) &= \int_0^x f(t)dt=\frac{\lambda^\alpha}{\Gamma(\alpha)}\int_0^x t^{\alpha-1}e^{-\lambda t} dt 
\end{array} \]
\pause
...
When should we expect the prize to be awarded? (intentionally vague)
...
round(qgamma(c(0.025,.975), shape=alpha, rate=lambda),1)
@
...
Let $X_1,\ldots,X_n \ind N(m_n,s_n^2)$. Then 
\[
Z = \frac{X_i-m_i}{s_i} \sim N(0,1) \quad\mbox{for all } i
\]
and 
\[ 
Y = \sum_{i=1}^n X_i \sim N\left(\sum_{i=1}^n m_n, \sum_{i=1}^n s_n^2 \right).
\]
...
If $X\sim N(15,2)$ what is $P(X>18)$? \pause
\[ \begin{array}{rl}
P(X>18) &= 1-P(X\le 18) \\
