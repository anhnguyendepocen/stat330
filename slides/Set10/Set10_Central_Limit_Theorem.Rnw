...
graphics:::plot.histogram(list(mid = x, 
															 density = dbinom(x,n,p),
															 breaks = c(x,n+1)-.5,
															 counts = dbinom(x,n,p)),
													main = "Probability mass/density functions",
													xlab = "x",
													ylab = "p(x)")
...
Then 
\[ \begin{array}{rl|}
P(Y\ge 50) &= 1- P(Y\le 49) = \sum_{y=0}^{49} {n\choose y} p^y (1-p)^{n-y} \\
&=\approx P\left(Z\ge \frac{50-np}{\sqrt{np(1-p)}}\right) 
\end{array} \]
\pause
<<echo=TRUE>>=
n = 99
