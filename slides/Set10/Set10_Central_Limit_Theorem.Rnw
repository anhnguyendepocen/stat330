...
&= 0.399048 \\
\\
&\approx P(X\ge 50-0.5) & \mbox{continuity correction}\\ 
&= P\left(Z\ge \frac{49.5-np}{\sqrt{np(1-p)}}\right) \\
&= P(Z\ge 0.2552089) \\
...
1-pnorm((50  -n*p)/sqrt(n*p*(1-p))) # Standardized
1-pnorm((49.5-n*p)/sqrt(n*p*(1-p))) # Standardized with continuity correction
1-pnorm(50, mean=n*p, sd= sqrt(n*p*(1-p)))     # Original scale
1-pnorm(49+0.5, mean=n*p, sd= sqrt(n*p*(1-p))) # Original scale with continuity correction
