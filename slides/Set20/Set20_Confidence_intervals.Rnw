...
\begin{frame}
\frametitle{Bernoulli proportion confidence interval}
...
<<die_rolling_cis, dependson="die_rolling_data", echo=FALSE, results='asis'>>=
...
with
\[ 
E\left[\hat{\theta}\right] = \theta_X - \theta_Y 
\quad\mbox{and}\quad
Var\left[\hat{\theta}\right] = \frac{\theta_X(1-\theta_X)}{n_X} + \frac{\theta_Y(1-\theta_Y)}{n_Y}.
\]
...
tab <- xtable(d_format, align="cccc", digits=0)
print(tab, type="latex", include.rownames = FALSE)
@

<<web_hosting_ci, dependson="web_hosting_data", echo=FALSE>>=
a = 0.05
d = d %>%
  mutate(theta_hat = y/n,
         var = theta_hat*(1-theta_hat)/n)

xbar = round(d$theta_hat[1],2)
ybar = round(d$theta_hat[2],2)
...
A $\Sexpr{100*(1-a)}$\% confidence interval for the difference (A-B) is 
\[ 
\Sexpr{xbar} - \Sexpr{ybar} \pm \Sexpr{round(z_crit,2)}\sqrt{\frac{\Sexpr{xbar}(1-\Sexpr{xbar})}{n_X} + \frac{\Sexpr{ybar}(1-\Sexpr{ybar})}{n_Y}}
= (\Sexpr{round(L,2)}, \Sexpr{round(U,2)}).
\]
