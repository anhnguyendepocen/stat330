...
\begin{frame}
\frametitle{Bernoulli probability confidence interval}
...
\begin{frame}
\frametitle{Difference in normal means}
...
\begin{frame}
\frametitle{Difference in heights}
...
tab <- xtable(d_format, align="ccccc", digits=c(NA,NA,0,1,1))
print(tab, type="latex", include.rownames = FALSE)
@
