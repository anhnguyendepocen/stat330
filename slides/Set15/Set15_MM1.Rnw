...
\[ 
E[X_s] = \lambda_A E[S] = \lambda_A/\lambda_S = r
\]
...
\begin{itemize}
\item Expected number of jobs in the system \pause
\[
E[X] = \frac{r}{1-r} = \frac{\frac{3}{4}}{1-\frac{3}{4}} = 3 \mbox{ jobs}
\]
\pause
\item Expect number of jobs being processed \pause
\[ 
E[X_s] = r = \frac{3}{4} \mbox{ jobs}
\]
\pause
\item Expected number of jobs in the queue \pause
\[ 
E[X_w] = E[X]-E[X_s] = 3-\frac{3}{4} = 2\frac{1}{4} \mbox{ jobs}
\]
\pause
\item Expected total time the job is in the system \pause
\[ 
E[R] = E[X]/\lambda_A = 3/3 = 1\mbox{ hour}
\]
\pause
\item Expected time the job is in the queue \pause
\[ 
E[W] = E[X_w]/\lambda_A = \frac{2\frac{1}{4}}{3} = 0.75 \mbox{ hours}
\]
\end{itemize}
...
\begin{definition}
An \alert{$M/M/1/K$} queuing process is an $M/M/1$ queuing process with a maximum 
of $K$ for the system with \emph{attempted} arrival rate $\lambda$ and service 
rate $\lambda_S$.
\end{definition}
...
\[ 
\pi_k = r^k\pi_0, \quad \pi_0 = \frac{1-r}{1-r^{K+1}}.
\]
where $r=\lambda/\lambda_S$. 
\pause 
The average line length is 
\[
E[X] = \sum_{x=0}^K k \pi_k = \cdots 
= \frac{r}{1-r}-\frac{(K+1)r^{K+1}}{1-r^{K+1}}.
\]
The probability the queue is full is $p_K$, thus the actually arrival rate is 
\[ 
\lambda_A = (1-p_K)\lambda
\]
since no jobs can arrive while the queue is full. 
\end{frame}
...
\begin{frame}
\frametitle{Remaining properties for $M/M/1/K$}
\tiny

\begin{itemize}
\item Expected service time
\[ 
E[S] = \frac{1}{\lambda_S} = \mu_S
\]
\pause
...
\[ 
E[X_s] = \lambda_A E[S] = \frac{\lambda_A}{\lambda_S} 
...
\[
E[W] = E[R]-E[S] = \frac{E[X]}{\lambda_A}- \frac{1}{\lambda_S}
\]
\end{itemize}
...
K = 5
lambda = 3
lambda_S = 4; Ws = 1/lambda_S # E[S]
r = lambda/lambda_S
pi0 = (1-r)/(1-r^(K+1))
pi = numeric(K); for (k in 1:K) pi[k] = pi[k] = r^k*pi0
pi[K]
lambda_A = (1-pi[K])*lambda
L = sum(1:5*pi)  # E[X]
W = L/lambda_A   # E[R]
Wq = W-Ws        # E[W]
Lq = Wq*lambda_A # E[X_w]
Ls = L-Lq        # E[X_s]
round(c(L, Ls, Lq, W, Ws, Wq),2)
 @
