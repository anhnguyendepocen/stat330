...
\begin{definition}
An \alert{$M/M/1/K$} queuing process is an $M/M/1$ queuing process with a 
...
\[
E[X] = \sum_{x=0}^K x \pi_x = \cdots 
= \frac{r}{1-r}-\frac{(K+1)r^{K+1}}{1-r^{K+1}}.
\]
\pause
The probability the queue is full is $\pi_K$, \pause thus the actually arrival rate is 
\[ 
\lambda_A = (1-\pi_K)\lambda
\]
...
\item Expected number of jobs in the system
\[ 
E[X] = \frac{r}{1-r}-\frac{(K+1)r^{K+1}}{1-r^{K+1}}
\]
\item Expected service time
\[ 
...
r = lambda/lambda_S
pi0 = (1-r)/(1-r^(K+1))
pi = numeric(K); for (k in 1:K) pi[k] = r^k*pi0
lambda_A = (1-pi[K])*lambda
round(c(pi[K],lambda_A),2)
L = sum(1:5*pi)  # E[X]
W = L/lambda_A   # E[R]
Wq = W-Ws        # E[W]
