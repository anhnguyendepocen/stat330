...
\title{Set 24 - Bayesian model comparison}
...
\begin{itemize}
\item Simple hypotheses can be considered to have a point-mass prior, \pause e.g. if $H_0:\theta=\theta_0$ then $P(\theta=\theta_0|H_0)=1$. \pause Then the marginal likelihood is 
\[
p(y|H_0) = p(y|\theta_0).
\]
\item \pause Composite hypotheses have a continuous prior and thus the 
marginal likelihood is
\[
p(y|H_j) = \int p(y|\theta)p(\theta|H_j) d\theta
\]
for some prior $p(\theta|H_j)$.
\end{itemize}
...
\frame{\frametitle{Binomial model}
  ...
  \pause 
  With $p(H_0)=p(H_1)$, the posterior model probability is 
