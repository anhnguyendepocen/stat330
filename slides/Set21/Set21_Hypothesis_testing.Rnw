...
\pause
Test the hypothesis $H_0: \mu = \Sexpr{mu0}$ GHz. 
...
% \begin{frame}
% \frametitle{Power example}
% 
% \small
% 
% In our CPU fabrication facility example, suppose the actual average speed is
% 1.78 GHz (which is below our specification of 1.8 GHz).
% \pause
% If we set the significance level to $a=0.05$, what is the power of the test when
% 14 observations are taken?
% 
% \vspace{0.1in} \pause
% 
% In order to reject the test, we need the absolute value of our test statistic
% to be greater than 2.145 since
% \[
% 0.05 = 2P(T_{14} > 2.145).
% \]
% \pause
% Assuming $\mu=1.78$ GHz and $\sigma=0.05$ GHz, we have
% \[
% T'_{n-1} = \frac{\overline{Y}-\mu}{\sigma/\sqrt{n}} \sim t_{n-1}.
% \]
% \pause
% Thus we have
% \[ \begin{array}{rl}
% \mbox{power}
% &= P(\mbox{reject } H_0|H_0 \mbox{ is not true}) \\
% &=
% \end{array} \]
% 
% \end{frame}
...
Let $Y_i$ be the clock speed for chip $i$ and assume $Y_i\ind N(\mu,\sigma^2)$. 
\pause
Test the hypothesis $H_0: \mu \ge \Sexpr{mu0}$ GHz. 

\vspace{0.1in} \pause

We previously found the estimator $\hat\mu = \Sexpr{ybar}$ and the pvalue for 
the test $H_0 = \Sexpr{mu0}$ GHz as
\[ 
p = 2P(T_{14} > |\Sexpr{t}| \approx \Sexpr{round(p,3)}
\]
Since $\Sexpr{ybar} < \Sexpr{mu0}$, we divide this pvalue in half and thus our
one-sided pvalue is 
\[
