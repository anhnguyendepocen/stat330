...
\title{Set 27 - Adavnced Regression}
...
\[ 
Y_i \ind N(\beta_1+\beta_1 x_{i,1} + \cdots + \beta_q x_{i,q}, \sigma^2).
\]
\pause
...
\quad \mbox{or} \quad
Y_i = x_i^\top + e_i, \quad e_i \ind N(0,\sigma^2).
\]
...
\quad \mbox{or} \quad
Y_i = X\beta + e, \quad e \sim N(0,\sigma^2\mathrm{I})
\]
...
= \frac{r^\top r}{n-p}.
 \]
\pause
You can get similar to confidence intervals and prediction intervals to what
we had before. 
\end{frame}
...
<<multiple_regression_model2, dependson="multiple_regression_model">>=
...
Random forests resample the data with replacement and construct a tree from 
