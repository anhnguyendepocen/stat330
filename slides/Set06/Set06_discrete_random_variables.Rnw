...
dragonwood3 = data.frame(x   = round(as.numeric(names(tsum)),3),
                         pdf = round(as.numeric(table(sum)/length(sum)),3)) %>%
	mutate(cdf = cumsum(pdf))

t(dragonwood3)
# round(dragonwood3$pdf,3)
...
\begin{frame}
\frametitle{Dragonwood - pdf and cdf}

<<dragonwood_plots, dependson='dragonwood', fig.width=8>>=
g_pdf = ggplot(dragonwood3, aes(x=x, y=pdf)) + 
	geom_point() + 
	scale_x_continuous(breaks=3:12) + 
	ylim(0,1)

# set  up cdf plot
cdf = bind_rows(data.frame(x=2,pdf=0,cdf=0), dragonwood3) %>% 
...
grid.arrange(g_pdf,g_cdf, nrow=1)
@
...
expectation = with(dragonwood3, sum(x*pdf))
expectation
@
...
ggplot(dragonwood3 %>% mutate(y=0), aes(x=x, xend=x, y=y, yend=pdf)) + 
	geom_segment(size=2) + 
  geom_point(data = data.frame(x=7.5,y=-0.01,pdf=0), aes(x,y), shape=24, color='red', fill='red') + 
	scale_x_continuous(breaks=3:12)
@
\end{frame}
...
d = data.frame(x=c(0,1), pdf=c(0.1,0.9))

ggplot(d %>% mutate(y=0), aes(x=x, xend=x, y=y, yend=pdf)) + 
	geom_segment(size=2) + 
  geom_point(data = data.frame(x=0.9,y=-0.04,pdf=0), aes(x,y), shape=24, color='red', fill='red') + 
	scale_x_continuous(breaks=3:12) + 
	labs(x='y', y='pdf')
@
\end{frame}
...
variance = with(dragonwood3, sum((x-expectation)^2*pdf))
variance
@
