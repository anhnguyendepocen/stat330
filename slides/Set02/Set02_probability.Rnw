...
\item Review symbols $\in, \notin, \subset, \subseteq, \supset, \supseteq$.
  ...
  For example,
  \begin{itemize}
  \item if a is a member of B, this is denoted $a  \in B$ \pause and
...
numbers $P(A)$ to events $A \subset \Omega$ \pause such that
...
\begin{frame}
\frametitle{Solution}
Let  $A\equiv \text{WiFi up};\ B\equiv \text{ISP up}$
\begin{enumerate}
\item What is the probability that the WiFi is up or the ISP is up?
\[ P( \text{ WiFi up or ISP up}) =P(A\cup B) = 0.9 + 0.6 - 0.55 = 0.95 \]
\item  What is the probability that both the WiFi and the ISP are down?
\[ \begin{array}{rl}
P( \text{ WiFi down and ISP down}) &=P(\bar A\cap \bar B) = P(\overline{A\cup B}) \\
&=1-.95=.05
\end{array} \]
\item What is the probability that we fail to connect?
\[ \begin{array}{rl}
...
&=P(\bar A\cup \bar B) =(1-.9)+(1-.6)-.05=.1+.4-.05=.45
\end{array} \]
\end{enumerate}
