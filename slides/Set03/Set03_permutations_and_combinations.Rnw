...
\begin{theorem}
If elementary outcomes in a sample space are equally likely, \pause i.e. $P(\{ \mOmega \}) $ is the same for all $\mOmega \in \mOmega$), \pause then  the probability of an event $A$ is given by:
\[ P(A) = \frac{|A|}{|\mOmega|}, \]
\pause
...
Let us compute this using probability theory:
\begin{itemize}[<+->]
\item One possible sample space $\mOmega$ is: $\mOmega = \{ (g_{1}, g_{2}),  (g_{1}, g_{3}), (g_{1}, d),(g_{2}, g_{3}), (g_{2}, d), (g_{3}, d) \}$
\item The event to draw the defective chip is then $E  = \{ (g_{1}, d), (g_{2}, d), (g_{3}, d) \}$.
\item We can write the probability to draw the defective chip by comparing 
the sizes of $E$ and $\mOmega$:
\[
P(A) = \frac{|E|}{|\mOmega|} = \frac{1}{2}.
\]
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Break the complex action into a series $k$ single draws, i.e. $x_{i}$ is outcome on draw $i$. 
...
\item {\textcolor{cyan}{One of the choices is ``olives''. If all possible rankings are equally likely, what is the probability that a randomly selected survey has ``olives'' in the top 3?}} % P(6,3)/P(7,3) = 120/210 = 0.57
...
\begin{definition}
A \emph{combination} is a selection of all or part of a set of objects without regard to the order in which objects are selected. 
\end{definition}
...
\begin{theorem}
The number of combinations of $k\le n$ objects chosen from $n$ objects is 
...
\item What is the probability that you win? \pause % ??? [C(5,3)+C(5,4)+C(5,5)]/C(49,5) \approx 50/1906884 \approx 2.6e-5
	\begin{enumerate}
...
\item $55^8 \approx 84$ trillion 8 character passwords with $26\times 2+3$ character dictionary. \pause
\item There are $45^8$ all letter or special character passwords and $10^8$ all number passwords for a total of about 17 trillion invalid passwords. \pause
\item Thus, there are approximately $84-17 = 67$ trillion possible passwords. 
\end{itemize}
}
