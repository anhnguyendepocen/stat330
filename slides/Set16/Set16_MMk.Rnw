...
pi_x = function(x,k,r) {
	pi0 = pi_0(k,r)
	pi0 * ifelse(x<=k, r^x/factorial(x), r^k/factorial(x)*(r/k)^(x-k))
}
...
\begin{example}
Suppose the call center can only handle 100 jobs in the system. 
Now what are the system characteristics?
\end{example}
...
pi_0 = function(k, r, K) {
	ii = 0:(k-1); II = k:K
	1/(sum(r^ii/factorial(ii)) + sum(r^k/factorial(k)*(r/k)^II-k))
}
...
K = 100
pi0 = pi_0(k,r,K)
pi = numeric(K); for (kk in 1:K) pi[k] = pi_x(kk, k, r, K)
