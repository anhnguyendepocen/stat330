...
\[ \begin{array}{rl}
p_{x,x+1} &= P(\mbox{1 A, 0 D}) \\
&= p_A(1-p_S)^n \\ \\
p_{x,x} &= P(\mbox{1 A, 1 D}) + P(\mbox{0 A, 0 D}) \\
&= p_A {n\choose 1} p_S(1-p_S^{n-1}) + (1-p_A)(1-p_S)^n \\ \\
p_{x,x-1} &= P(\mbox{1 A, 2 D}) + P(\mbox{0 A, 1 D}) \\
&= p_A {n\choose 2} p_S^2(1-p_S^{n-2}) + (1-p_A)p_S(1-p_S)^{n-1} \\
& \qquad \vdots \\
p_{x,x-n} &= P(\mbox{0 A, $n$ D}) \\
&= (1-p_A)p_S^n
\end{array} \]
...
\left\{ 
\begin{array}{cl}
\pi_0 &= \left( \sum_{i=0}^{k-1} \frac{r^i}{i!} + \sum_{i=k}^C \frac{r^k}{r!}(r/k)^{x-k} \right)^{-1} \\
\pi_x &= \left\{ 
\begin{array}{ll}
\frac{r^x}{x!}\pi_0 & \mbox{for }x\le k \\
\frac{r^k}{k!}\pi_0 \left( \frac{r}{k} \right)^{x-k} & \mbox{for }k<x\le C
\end{array}
\right.
\end{array}
\right. 
...
This is exactly the same as in the $M/M/1/C$ queuing process. 
...
expectations$new_value = round(c(X, X_w, X_s, R, W, S), 1)
...
With an infinite number of servers, there is no queue and thus
\[
X_w = 0, \, W=0, \, X = Xs, \mbox{ and }R = S.
\]
\pause
The characteristics of this system can be obtained by taking the limit as 
$k\to\infty$. \pause
...
\]
\pause
This is fairly close to the results with $k=55$ because with $k=\Sexpr{k}$ 
servers very few jobs have to wait, 
$E[X_w]=\Sexpr{expectations$value[expectations$characteristic=="jobs_waiting"]}$
jobs, and those that wait, don't have to wait very long, 
$E[W] = \Sexpr{expectations$value[expectations$characteristic=="time_waiting"]}$
 seconds.
