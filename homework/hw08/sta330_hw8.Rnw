...
\item Suppose $Y_i \stackrel{ind}{\sim} Po(\lambda)$ for $i=1,\ldots, n$.
  ...
  \item If we observe the data, 7, 6, 7, 2, and 4, what are the values of the method of moments and maximum likelihood estimator for $\lambda$?
  \end{enumerate}
...
\item A sample of 3 observations of waiting time to access an internet server is $x_1=0.4, x_2=0.7, x_3=0.9$ seconds. It is believed that the waiting time has the continuous distribution
\[ f(t)=\begin{cases}\ \theta t^{\theta-1}, \ & 0<t<1\\
\ 0,\ & \text{otherwise}\end{cases} \]
\begin{enumerate}
\item Find an estimate of the parameter $\theta$ using the method of moments.
\item Find the maximum likelihood estimate of $\theta$.
\end{enumerate}
...
\end{align*}
 \end{enumerate}
  ...
  \item Let $X_1,\dots, X_n$ be a random sample from the gamma distribution with $\alpha=3$.
    ...
    for $x\ge 0$.
    \begin{enumerate}
      \item Find an estimate of the parameter $\lambda$ using the method of moments.
      \item Find the maximum likelihood estimate of $\lambda$.
    \end{enumerate}
      ...
      \begin{enumerate}
        ...
        \item 
          \begin{align*}
            ...
            \hat{\lambda} &= \frac{\sum^n_{i=1}x_i}{3n} = \frac{3}{\bar{x}}
          \end{align*}
      \end{enumerate}
    }
\end{enumerate}
